...
* You can submit multiple parallel corpus files by repeating the parallel\_corpus multipart form parameter as long as the cumulative size of the files doesn't exceed 250 MB.
...
3. Identify if a specific model, for example `en-fr`, supports customization, execute command
...
5. Optionally, you may execute the command below and retrieve all models for customization support.
...
7. Train your custom model.
   ...
   Use the `Create model` method to train your model. In your request, specify the model ID of a customizable base model, and training data in the parallel\_corpus parameters.
   ...
   You can upload multiple parallel\_corpus files in one request. All uploaded parallel\_corpus files combined, your parallel corpus must contain at least 5,000 parallel sentences to train successfully.
...
14. You can apply a `forced glossary` to a model that has been customized with a parallel corpus.
...
16. Record the new model ID.

    ```text
     export MODELID2=<new model ID>
    ```

17. Check the status of the new custom model.
...
18. When the model status is available, your model is ready to use with your service instance.
...
19. Translate text with your new custom model.
...
21. Deleting a custom translation model. **Don't run the command if you plan to use the custom model again**.
